...
Mathematica tables are formed by sets of curly braces. Each set of braces creates a layer of the table. A one-dimensional table, or a list, follows the form. A two-dimensional table follows the form. This specific table has two rows and three columns. Each element in the outer braces is a row element, and each element in the inner braces is a column element. A three-dimensional table follows the form. This table has two rows, three columns, and two layers.
